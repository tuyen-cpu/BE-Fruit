...
public interface RevenueRepo extends JpaRepository<Order,Long>{
	...
	@Query("select new com.example.befruit.dto.RevenueDTO(cast(o.createdAt as date),cast(count(o) as integer ) ,sum(o.total + o.shippingCost),sum(o.shippingCost),sum(o.total)) from Order o where (day(cast(o.createdAt as date))>=?1 and day(cast(o.createdAt as date))<=?2) and (month(cast(o.createdAt as date))>=?3 and month(cast(o.createdAt as date))<=?4 )and (year(cast(o.createdAt as date))>=?5 and year(cast(o.createdAt as date))<=?6) group by day(cast(o.createdAt as date)),month(cast(o.createdAt as date)),year(cast(o.createdAt as date))")
	...
	@Query("select new com.example.befruit.dto.RevenueDTO(DATE_FORMAT(cast(o.createdAt as date), '%m/%Y'),cast(count(o) as integer ) ,sum(o.total + o.shippingCost),sum(o.shippingCost),sum(o.total)) from Order o where (month(cast(o.createdAt as date))>=?1 and month(cast(o.createdAt as date))<=?2 )and (year(cast(o.createdAt as date))>=?3 and year(cast(o.createdAt as date))<=?4) group by month(cast(o.createdAt as date)),year(cast(o.createdAt as date))")
